...
export const CreateProblemPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(`/problems/${createdProblem.slug || createdProblem.id}`);
    } catch (err: any) {
      const msg =
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[var(--border)] pb-5">
        <div className="space-y-1">
          ...
          <p className="text-sm text-[var(--text-3)]">
            Define problem statements, algorithmic constraints, test cases, and starter code templates.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <Link to="/problems" className="btn-secondary !text-xs !py-2">
            Cancel
          </Link>
          <button
            type="button"
